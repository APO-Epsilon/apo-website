...
function refresh(){
	echo("<meta http-equiv=\"REFRESH\" content=\"0;url=service_dashboard.php\">");
}
	...
	if(!$resultO){
		...
		while($r = mysqli_fetch_array($resultO)){
			...
			if(($count < $max)||($num_rows == 1)||($max==-1)){
				if($num_rows == 0){
					$m = 1;
					$message = "<a href=\"service_dashboard.php?d=$detail_id&o=$occurrence_id\">sign-up</a>";
					//$message = "";
				}else{
					$m = 2;
					$message = "<a href=\"service_dashboard.php?r=$detail_id&o=$occurrence_id\">remove</a>";	
					$message;
				}
			}else{
				$message = "full";
